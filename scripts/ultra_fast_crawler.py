...
def generate_item():
    ...
    # 添加更多变化
    prefixes = ["", "用户问：", "咨询：", "问题：", "求助：", "请问："]
    suffixes = ["", "求指导", "怎么办", "如何处理", "求建议"]
    
    q_varied = random.choice(prefixes) + q + random.choice(suffixes)
    ...
    while not stop_flag.is_set() or not data_queue.empty():
        try:
            ...
            if time.time() - last_report > 10:
                print(f"已保存 {count} 条 | 总计 {total_count} 条")
                last_report = time.time()
...
def save_batch(items):
    ...
    filename = f"ultra_{timestamp}.jsonl"
...
def main():
    print("=" * 60)
    print(f"超高速数据生成器 - {NUM_WORKERS} 线程")
    print("=" * 60)
